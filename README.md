...
项目能部署到 Vercel 等 Next.js 平台，但 SQLite 文件不适合多实例和临时文件系统。设置 `DATABASE_URL` 后，缓存与景区位置库都会自动切换到 PostgreSQL，并在首次启动时执行带校验和与并发锁的迁移；未设置时使用 SQLite。地点缓存为 24 小时，天气缓存为 30 分钟。
