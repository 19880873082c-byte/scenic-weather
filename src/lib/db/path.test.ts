...
describe("database runtime configuration", () => {
  it("allows local SQLite and explicit persistent SQLite paths", () => {
    expect(getDatabaseRuntimeStatus({})).toMatchObject({ backend: "sqlite", configured: true });
    expect(getDatabaseRuntimeStatus({ VERCEL: "1", SQLITE_PATH: "/data/app.sqlite" })).toMatchObject({ backend: "sqlite", configured: true });
  });
